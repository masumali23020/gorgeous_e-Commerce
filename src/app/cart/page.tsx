    ...
    <div className="flex flex-col items-center justify-center mt-12">
        ...
        {steps.map((step) => (
          <div
            ...
            className={`flex items-center gap-2 border-b-2 pb-4 ${
              step.id === isActive ? "border-gray-800" : "border-gray-200"
            }`}
          >
            <div
              className={`flex items-center justify-center w-6 h-5 rounded-full p-4 ${
                step.id === isActive ? "bg-gray-800" : "bg-gray-300"
              } text-white`}
            >
              {" "}
              {step.id}
            ...
            <p
              className={`text-sm font-medium ${
                step.id === isActive ? "text-gray-800" : "text-gray-400"
              }`}
            >
              {step.title}{" "}
            </p>
      ...
      <div className="w-full flex flex-col lg:flex-row gap-16 mt-4">
        ...
        <div className="w-full lg:w-7/12 shadow-lg border-1 border-gray-100 rounded-lg flex flex-col gap-8 ">
            ...
            <div>
              {cartItems.map((item) => (
                ...
                >
                  ...
                  {/* delete button  */}
                  <button
                    onClick={() => handelDelteCart(item)}
                    className="w-8 h-8 rounded-full bg-red-100 hover:bg-red-200 transition-all duration-300 text-red-400 flex justify-center items-center cursor-pointer"
                  >
          ...
          ) : isActive === 3 && shippingForm ? (
            <PaymentForm />
          ) : (
            <p className="text-sm text-red-500">
              please fill in the shiooing form to continue.
            </p>
          )}
        </div>
        {/* left side  */}
